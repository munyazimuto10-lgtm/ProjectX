...
import { isAuthRelaxMode, PROJECTX_PORTAL_ROLE_KEY, roleFromUser } from '@/lib/auth';

export function Authentication() {
  const [authScreen, setAuthScreen] = useState<'role-select' | 'login' | 'forgot'>('role-select');
  ...
  const verifyRoleOrSignOut = async (intent: AppRole): Promise<boolean> => {
    ...
    if (actual !== intent) {
      setError(`This account is registered as ${actual}, not ${intent}. Use the correct portal or update user metadata.`);
      await supabase.auth.signOut();
      return false;
    }
    return true;
      ...
      <div className="w-full max-w-md">
        ...
        {authScreen === 'role-select' && (
          <div className="space-y-4">
            ...
            <button
              type="button"
              onClick={() => { setSelectedRole('admin'); setAuthScreen('login'); setError(''); }}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1">
                  ...
                  <p className="text-sm text-gray-600">Payroll management (requires role &quot;admin&quot; in user metadata)</p>
                </div>
            ...
            <button
              type="button"
              onClick={() => { setSelectedRole('employee'); setAuthScreen('login'); setError(''); }}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1">
                  ...
                  <p className="text-sm text-gray-600">Payslips & profile (requires role &quot;employee&quot;)</p>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8">
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => { setAuthScreen('forgot'); setError(''); setResetSent(false); }}
              ...
              <button
                type="button"
                onClick={() => { setAuthScreen('role-select'); setSelectedRole(null); setError(''); }}
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <p className="text-sm text-center text-gray-600">
                Supabase signs you in with a real session so RLS policies can use <code className="text-xs bg-gray-100 px-1 rounded">auth.uid()</code>.
              </p>
            </div>
          </div>
        )}
            ...
            {!resetSent ? (
              <>
                ...
                <form onSubmit={handleForgotPassword} className="space-y-6">
                  ...
                  <button
                    type="button"
                    onClick={() => { setAuthScreen('login'); setResetEmail(''); setError(''); }}
                    className="w-full flex items-center justify-center gap-2 text-gray-600 hover:text-gray-800 text-sm"
                  >
                    <ArrowLeft className="w-4 h-4" />
                    Back to login
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mb-6">
                  If an account exists, we sent instructions to <strong>{resetEmail}</strong>
                </p>
                <button
                  type="button"
                  onClick={() => { setAuthScreen('login'); setResetEmail(''); setResetSent(false); }}
