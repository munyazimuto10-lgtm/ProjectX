...
## Recommended Cloud Database: Supabase (PostgreSQL)

### Why Supabase?

**Advantages:**

- **Built on PostgreSQL:** Industry-standard, reliable RDBMS
- **Real-time subscriptions:** Instant updates to the UI
- **Row Level Security (RLS):** Built-in data access control
- **Auto-generated REST API:** No need to write CRUD endpoints
- **Authentication built-in:** Email, OAuth, MFA support
- **File Storage:** Integrated storage for payslips and documents
...
- **Team ($599/mo):** Custom resources + support

### Alternative Options

1. **Amazon RDS (PostgreSQL)**
   - More control and customization
   - Better for enterprise at scale
...
└──────────────┘   │   └──────────────────┘   │   └─────────────┘
        ...
        │                  │               │               │
┌───────▼──────┐  ┌────────▼─────┐ ┌──────▼───────┐ ┌─────▼──────┐
│  deductions  │  │   tax_forms  │ │   payslips   │ │audit_logs  │
└──────────────┘  └──────────────┘ └──────────────┘ └────────────┘
```
...
INSERT INTO users (email, password_hash, role) VALUES
    ('admin@payrollpro.com', crypt('admin123', gen_salt('bf')), 'admin'),
    ('accountant@payrollpro.com', crypt('accountant123', gen_salt('bf')), 'accountant');

-- Insert test employees
INSERT INTO employees (
    employee_id, first_name, last_name, email, phone, hire_date,
    department_id, position, base_salary, employment_status
) VALUES
    ('EMP-001', 'Sarah', 'Johnson', 'sarah.johnson@company.com', '+1-555-0101', '2020-01-15',
     (SELECT id FROM departments WHERE code = 'ENG'), 'Senior Developer', 8500.00, 'active'),
    ('EMP-002', 'Michael', 'Chen', 'michael.chen@company.com', '+1-555-0102', '2021-03-20',
     (SELECT id FROM departments WHERE code = 'SAL'), 'Sales Manager', 7500.00, 'active'),
    ('EMP-003', 'Emily', 'Rodriguez', 'emily.rodriguez@company.com', '+1-555-0103', '2019-08-10',
     (SELECT id FROM departments WHERE code = 'MKT'), 'Marketing Director', 9000.00, 'active');
```
...
1. **Export schema from development:**
   ...
   ```bash
   pg_dump -s -h localhost -U postgres payrollpro_dev > schema.sql
   ```

2. **Apply to production:**

   ```bash
   psql -h your-supabase-url -U postgres -d postgres < schema.sql
   ```
...
```bash
# Backup database
pg_dump -h db.xxxxx.supabase.co -U postgres payrollpro > backup.sql

# Restore database
psql -h db.xxxxx.supabase.co -U postgres payrollpro < backup.sql
```
...
3. **Encrypt sensitive data** (e.g., tax IDs, bank accounts)
